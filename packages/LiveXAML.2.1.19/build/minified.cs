...
){I(a,e);L.WriteLine("Failed to call ctor: "+e.Message);return false;}}private
static void y(Element a,Type b,Element c){var d=b.GetRuntimeFields();foreach(var
fld in d){try{if(!fld.IsLiteral&&!fld.IsStatic){var f=fld.GetValue(c);if(f!=null
)fld.SetValue(a,f);}}catch(Exception){L.WriteLine("Couldn't copy field "+fld.
Name);}}}private static void z(Element a,Element b){var c=a.GetType().
GetTypeInfo();var d=new List<TypeInfo>();d.Add(c);while(c.BaseType!=null){c=c.
BaseType.GetTypeInfo();d.Add(c);}var f=new HashSet<string>();for(int g=d.Count-1
;g>=0;g--){var h=d[g].AsType();var i=J(h);if(i==null)continue;if(f.Contains(i))
continue;f.Add(i);var j=h.GetRuntimeProperties().FirstOrDefault(k=>k.Name==i);
var l=j.GetValue(a);z(a,b,h,j,l);}}private static void z(Element a,Element b,
Type c,PropertyInfo d,object f){if(a is Grid){A((Grid)b,(Grid)a);}else if(f is
System.Collections.IEnumerable){var g=f.GetType().GetRuntimeMethods();var h=g.
FirstOrDefault(i=>i.Name=="Clear");var j=g.FirstOrDefault(i=>i.Name=="Add");if(h
==null||j==null)return;var k=d.GetValue(b)as System.Collections.IEnumerable;if(k
==null)return;h.Invoke(f,new object[0]);foreach(var newValue in k.OfType<object>
().ToList())j.Invoke(f,new object[]{newValue});var l=c.GetAllDeclaredMethods().
ToList();var m=l.FirstOrDefault(i=>i.Name=="OnChildrenChanged");if(m!=null){m.
Invoke(a,new object[]{a,new NotifyCollectionChangedEventArgs(
NotifyCollectionChangedAction.Reset)});}var n=l.FirstOrDefault(i=>i.Name==
"OnPagesChanged");if(n!=null){n.Invoke(a,new object[]{new
NotifyCollectionChangedEventArgs(NotifyCollectionChangedAction.Reset)});}}else{d
.SetValue(a,d.GetValue(b));}}private static void A(Grid a,Grid b){var c=a.
Children.ToArray();b.Children.Clear();a.Children.Clear();foreach(var child in c)
{var d=Grid.GetRow(child);var f=Grid.GetRowSpan(child);var g=Grid.GetColumn(
child);var h=Grid.GetColumnSpan(child);b.Children.Add(child,g,g+h,d,d+f);}}
private static void B(object a,object b){if(a is Page&&b is Page){((Page)a).
ToolbarItems.Clear();foreach(var newItem in((Page)b).ToolbarItems)((Page)a).
ToolbarItems.Add(newItem);}}private static void C(object a,FieldInfo[]b){var c=a
...
GetValue(a);if(i!=null)i.Invoke(a,EventArgs.Empty);}}private static void D(
...
}private static void E(object a){try{var b=a as Xamarin.Forms.BindableObject;if(
b==null)return;var c=b.BindingContext;if(c==null)return;b.BindingContext=null;b.
BindingContext=c;}catch(Exception e){L.WriteLine(
"Failed to refresh BindingContext: "+e.Message);}}private static void F(object a
...
MainPage.DisplayAlert("LiveXAML error",c,"OK");L.WriteLine(c);}private static
string J(Type a){TypeInfo b=a.GetTypeInfo();var c=b.CustomAttributes.
...
ArgumentType==typeof(string))return(string)f.Value;}if(b.BaseType!=null)return J
(b.BaseType);return null;}}internal static class P{private static readonly
Dictionary<string,Type>o=new Dictionary<string,Type>();private static readonly
Assembly[]p;static P(){try{p=s();}catch{L.o("Failed to load assemblies");p=new
...
.DefinedTypes)o[type.FullName]=type.AsType();}catch(System.Exception e){L.q(
...
private string u;private a4 v;private string w="192.168.0.12";private Listener(){p.
MessageReceived+=D;r=new Z(new[]{w,"127.0.0.1","169.254.80.80","10.0.2.2",
"10.0.3.2"},y,x);}private void x(){L.p("Failed to find find a handshake server"+
...
var a=new a6(t);a.BeginReceive(B,a);}private void y(string a){u=a;z(u);}private
void z(string a){var b=new a5();try{L.o("Listener connecting to "+a);b.s(a,s).
Wait(20);L.o("Listener connected to "+a);v=b.Client;A(v,0);}catch(Exception e){L
.o("Listener failed to connect to "+a);L.Exception(e);}}private void A(a4 a,int
b){try{if(b>=q.Length){b=0;}a.BeginReceive(q,b,q.Length-b,0,C,b);}catch(
Exception e){L.WriteLine("BeginReceive failed");L.Exception(e);z(u);}}private
void B(IAsyncResult a){a6 b=null;try{b=(a6)a.AsyncState;var c=new a2(a1.Any,t).
Object;var d=b.EndReceive(a,ref c);p.Feed(d,d.Length);b.BeginReceive(B,b);}catch
(Exception e){L.WriteLine("UdpEndReceive failed");L.Exception(e);if(b!=null)((
IDisposable)b.Object).Dispose();b=new a6(t);b.BeginReceive(B,b);}}private void C
(IAsyncResult a){try{var b=(int)a.AsyncState;var c=v.EndReceive(a);L.WriteLine(
"Listener received a message ("+c+")");if(c==0){L.WriteLine(
...
);}private static void q(Y a){s(a,Application.Current,0);r(a);L.q(
"Updating controls ended");if(!a.ElementFound){L.o("Updated element not found: "
+a.TypeName);}}private static void r(Y a){var b=P.FindType(
"Rg.Plugins.Popup.Services.PopupNavigation");if(b==null){L.q(
"Can't find Rg.Plugins.Popup.Services.PopupNavigation type");return;}L.q(
"Updating Rg Popup Pages");var c=b.FindProperty("PopupStack");if(c==null){L.q(
"PopupStack property was not found");return;}var d=c.GetValue(null)as
IEnumerable;if(d==null){L.q("Couldn't cast PopupStack to IEnumerable");}var f=d.
OfType<Element>();foreach(var element in f)s(a,element,0);L.q(
"Updating Rg Popup Pages ended");}private static X s(Y a,Element b,int c,
ListView d=null){if(a.TraversedObjects.Contains(b))return X.Normal();a.
TraversedObjects.Add(b);L.q(new string(' ',c*2)+b.GetType().Name);if(b is
ListView&&d==null)d=(ListView)b;if(b.GetType().FullName==a.TypeName){if(d!=null
&&d!=b){O.u(b.GetType(),a.XamlBuffer.XamlString);var f=v(d);if(f!=null){var g=
new Y(a.TypeName,new XamlBuffer("",new List<string>()),a.PropertyList);w(g,f);a.
ElementFound=true;return X.JumpToAncestor(f);}}w(a,b);return X.Normal();}
ResourceDictionary h=null;var i=N.GetMergedResourceDictionary(b,out h);if(i&&h.
GetType().FullName==a.TypeName){O.u(h.GetType(),a.XamlBuffer.XamlString);if(b is
VisualElement)N.CreateNewMergedResourceDictionary(((VisualElement)b).Resources);
else if(b is Application)N.CreateNewMergedResourceDictionary(((Application)b).
Resources);var j=v(d??b);if(j!=null){var g=new Y(a.TypeName,new XamlBuffer("",
new List<string>()),a.PropertyList);w(g,j);a.ElementFound=true;return X.
...
==1&&child!=n.Object)return n;}return X.Normal();}private static void t(Y a,
object b,int c,ListView d=null){if(b==null){L.q("Couldn't find NavigationProxy")
...
);return null;}private static void w(Y a,Element b){L.q("Updating control node "
+a.TypeName);a.ElementFound=true;if(!string.IsNullOrWhiteSpace(a.XamlBuffer.
XamlString)){O.u(b.GetType(),a.XamlBuffer.XamlString);ClearElement(b,a.TypeName,
a.PropertyList);O.InitializeComponent(b,a.XamlBuffer.XamlString);}else{O.
...
Application)a;c.Resources.Clear();}}catch(Exception e){L.WriteLine(
...
get;set;}}}internal class Z{private const int o=53031;private readonly string[]p
...
new AutoResetEvent(false);public Z(string[]a,Action<string>b,Action c){p=a.Where
(d=>!d.StartsWith("{")).ToArray();r=b;q=c;w().ContinueWith(f=>{if(f.Exception!=
null){L.o("ServerLocator error");L.o(f.Exception.ToString());}});}private async
Task w(){t++;if(t>p.Length-1){q();return;}var a=p[t];try{var b=new a5();L.o(
"Connecting to the handshake server "+a);var c=b.s(a,o);var d=await Task.WhenAny
(c,Task.Delay(500)).ContinueWith(f=>{if(f.Exception!=null){L.o(
"Handshake connection failed for "+a+" "+f.Exception.Message);return false;}if(f
.Result==null)return false;if(f.Result.Exception!=null){L.o(
"Handshake connection failed for "+a+" "+f.Result.Exception.Message);return
false;}if(f.Result!=c){L.o("Handshake connection timed out");return false;}
